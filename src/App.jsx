...
export default function App() {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      setTotalPosts(actualData.meta?.total || combinedPosts.length);
    } catch (err) {
  ...
  const renderPaginationButtons = () => {
    ...
    buttons.push(
      <button key="first" onClick={() => setPageNumber(1)}
        className={`${baseBtnClasses} ${pageNumber === 1 ? disabledBtnClasses : defaultBtnClasses}`}
        disabled={pageNumber === 1} aria-label="First page">&laquo;</button>
    );

    buttons.push(
      <button key="prev" onClick={() => setPageNumber(pageNumber - 1)}
        className={`${baseBtnClasses} ${pageNumber === 1 ? disabledBtnClasses : defaultBtnClasses}`}
        disabled={pageNumber === 1} aria-label="Previous page">&lsaquo;</button>
    );
    ...
    for (let i = startPage; i <= endPage; i++) {
      buttons.push(
        <button key={i} onClick={() => setPageNumber(i)}
          ...
          aria-current={pageNumber === i ? "page" : undefined}>{i}</button>
      );
    }

    buttons.push(
      <button key="next" onClick={() => setPageNumber(pageNumber + 1)}
        className={`${baseBtnClasses} ${pageNumber === totalPages ? disabledBtnClasses : defaultBtnClasses}`}
        disabled={pageNumber === totalPages} aria-label="Next page">&rsaquo;</button>
    );

    buttons.push(
      <button key="last" onClick={() => setPageNumber(totalPages)}
        className={`${baseBtnClasses} ${pageNumber === totalPages ? disabledBtnClasses : defaultBtnClasses}`}
        disabled={pageNumber === totalPages} aria-label="Last page">&raquo;</button>
    );
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8">
          ...
          <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 w-full md:w-auto">
            ...
            <div className="flex items-center justify-end md:justify-start">
              ...
              <div className="relative">
                <select id="sortBy" value={sortOrder} onChange={handleSortOrderChange}
                  className="block appearance-none w-full bg-white border border-gray-300 text-gray-700 py-2 px-4 pr-8 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200">
